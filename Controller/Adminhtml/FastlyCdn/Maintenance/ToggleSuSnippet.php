...
class ToggleSuSnippet extends Action
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            $this->commentAndWebHook($checkIfReqExist, $clone);
     ...
     * @throws LocalizedException
     */
    private function commentAndWebHook($checkIfReqExist, $clone)
    {
        if ($this->config->areWebHooksEnabled() && $this->config->canPublishConfigChanges()) {
            if ($checkIfReqExist) {
                $this->api->sendWebHook(
                    '*Maintenance Support snippet has been turned OFF in Fastly version '. $clone->number . '*'
                );
            } else {
                $this->api->sendWebHook(
                    '*Maintenance Support snippet has been turned ON in Fastly version '. $clone->number . '*'
                );
            }
        }

        $comment = ['comment' => 'Magento Module turned ON Maintenance Support snippet'];
        if ($checkIfReqExist) {
            $comment = ['comment' => 'Magento Module turned OFF Maintenance Support snippet'];
        }
